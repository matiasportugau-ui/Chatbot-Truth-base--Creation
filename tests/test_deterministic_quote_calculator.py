...
class TestDeterministicQuoteCalculator:
    def test_basic_isopanel_quote(self):
        result = calculate_panel_quote(
            panel_type="Isopanel",
            thickness_mm=50,
            # Must respect product min length from KB
            length_m=2.3,
            width_m=1.0,
            quantity=10,
        )
        line = result["line_items"][0]
        # area: 2.3m², unit: 2.3 * 41.88 = 96.324 -> 96.32, subtotal: 963.20
        assert line["area_m2"] == pytest.approx(2.3)
        assert line["unit_price_usd"] == 96.32
    ...
    def test_discount_application(self):
        ...
        # total: 7463.25
        line = result["line_items"][0]
        assert line["area_m2"] == pytest.approx(3.6)
        assert line["unit_price_usd"] == 165.85
