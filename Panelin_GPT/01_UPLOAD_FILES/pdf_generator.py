...
class QuotationDataFormatter:
    ...
    def calculate_totals(
        ...
        products_total = sum(
            item.get('total_usd') if 'total_usd' in item 
            else QuotationDataFormatter.calculate_item_total(item) 
        ...
        # Shipping
        shipping = shipping_usd if shipping_usd is not None else QuotationConstants.DEFAULT_SHIPPING_USD
...
class BMCQuotationPDF:
    ...
    def _build_title_section(self, data: Dict) -> List:
        ...
        elements.append(Paragraph(f"<b>Cliente:</b> {client.get('name', '')}", normal_style))
        elements.append(Paragraph(f"<b>Dirección:</b> {client.get('address', '')}", normal_style))
        elements.append(Paragraph(f"<b>Tel/cel:</b> {client.get('phone', '')}", normal_style))
    ...
    def _build_totals(self, totals: Dict) -> List:
        ...
        data = [
            ...
            [f"IVA {int(totals['iva_rate'] * 100)}%:", QuotationDataFormatter.format_currency(totals['iva'])],
            ['Materiales:', QuotationDataFormatter.format_currency(totals['materials_total'])],
            ['Traslado:', QuotationDataFormatter.format_currency(totals['shipping'])],
            ['TOTAL U$S:', QuotationDataFormatter.format_currency(totals['grand_total'])]
        ]
