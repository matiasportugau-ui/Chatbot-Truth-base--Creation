...
exports.jailbreak = exports.JailbreakOutput = exports.MAX_CONTEXT_TURNS = exports.JailbreakConfig = void 0;
 ...
 */
exports.JailbreakConfig = llm_base_1.LLMConfig;
/**
 ...
 */
exports.JailbreakOutput = llm_base_1.LLMOutput.extend({
...
function extractConversationHistory(ctx) {
    const candidate = ctx.getConversationHistory;
...
function buildAnalysisPayload(conversationHistory, latestInput) {
    const trimmedInput = typeof latestInput === 'string' ? latestInput.trim() : '';
    const recentTurns = conversationHistory.slice(-exports.MAX_CONTEXT_TURNS);
    return JSON.stringify({
        conversation: recentTurns,
        latest_input: trimmedInput,
    });
}
function isLLMErrorOutput(result) {
    return Boolean(result &&
        typeof result === 'object' &&
        'info' in result &&
        result.info &&
        typeof result.info === 'object' &&
        'error_message' in result.info);
}
/**
 * Conversation-aware jailbreak detection guardrail.
 */
const jailbreak = async (ctx, data, config) => {
    ...
    const analysis = await (0, llm_base_1.runLLM)(analysisPayload, SYSTEM_PROMPT, ctx.guardrailLlm, config.model, exports.JailbreakOutput);
    const usedConversationHistory = conversationHistory.length > 0;
    if (isLLMErrorOutput(analysis)) {
        return (0, llm_base_1.createErrorResult)('Jailbreak', analysis, {
...
exports.jailbreak = jailbreak;
registry_1.defaultSpecRegistry.register('Jailbreak', exports.jailbreak, 'Detects attempts to jailbreak or bypass AI safety measures using techniques such as prompt injection, role-playing requests, system prompt overrides, or social engineering.', 'text/plain', exports.JailbreakConfig, undefined, { engine: 'LLM', usesConversationHistory: true });
