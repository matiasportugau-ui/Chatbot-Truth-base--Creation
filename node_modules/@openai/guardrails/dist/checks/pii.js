...
exports.pii = exports.PIIConfigRequired = exports.PIIConfig = exports.PIIEntity = void 0;
 ...
 */
exports.PIIConfig = zod_1.z.object({
    entities: zod_1.z.array(zod_1.z.nativeEnum(PIIEntity)).default(() => Object.values(PIIEntity)),
    block: zod_1.z
        .boolean()
        .default(false)
        .describe('If true, triggers tripwire when PII is detected. If false, masks PII without blocking.'),
    detect_encoded_pii: zod_1.z
        .boolean()
        .default(false)
...
exports.PIIConfigRequired = zod_1.z
    .object({
    entities: zod_1.z.array(zod_1.z.nativeEnum(PIIEntity)),
    block: zod_1.z.boolean(),
    detect_encoded_pii: zod_1.z.boolean(),
})
    .transform((data) => ({
    ...data,
...
const BIC_WITH_CONTEXT_REGEX = new RegExp(`(?:${BIC_CONTEXT_PREFIX_PATTERN})[:\\s=]+([A-Z]{4}[A-Z]{2}[A-Z0-9]{2}(?:[A-Z0-9]{3})?)\\b`, 'g');
...
const KNOWN_BIC_REGEX = new RegExp(`\\b(?:${KNOWN_BIC_PREFIXES.join('|')})[A-Z]{2}[A-Z0-9]{2}(?:[A-Z0-9]{3})?\\b`, 'g');
/**
...
const DEFAULT_PII_PATTERNS = {
    ...
    [PIIEntity.LOCATION]: [
        {
            regex: /\b\d{1,6}\s[A-Za-z0-9\s]+(?:Street|St|Avenue|Ave|Road|Rd|Boulevard|Blvd|Drive|Dr|Lane|Ln|Place|Pl|Court|Ct|Way|Highway|Hwy|Parkway|Pkwy|Circle|Cir|Trail|Trl|Terrace|Ter)\b/gi,
        },
        {
 ...
 */
function _detectPii(text, config) {
    if (!text) {
...
function _normalizeUnicode(text) {
    if (!text) {
        return text;
    }
    try {
...
function _collectPlainDetections(text, entities) {
    const mapping = {};
    const spans = [];
    const seen = new Set();
...
function _detectEncodedPii(text, config) {
...
function _findEncodedCandidates(text) {
    const candidates = [];
    const seen = new Set();
    const addCandidate = (start, end, encodedText, decodedText, type) => {
        const key = `${start}:${end}`;
        if (seen.has(key)) {
            return;
        }
...
function _selectPreferredEntity(matchedEntities, priorityOrder) {
...
function _mergeDetectionSets(plain, encoded) {
    const merged = {};
...
function _convertSetsToArrays(mapping) {
    const result = {};
...
function _dedupeReplacements(replacements) {
...
function _applyReplacements(text, replacements) {
    let offset = 0;
    let result = text;
...
function _tryDecodeBase64(text) {
 ...
 * @throws Error if text is empty or null
 */
function _scrubPii(originalText, detection) {
    if (!originalText) {
        throw new Error('Text cannot be empty or null');
    }
 ...
 */
function _asResult(detection, config, name, text) {
 ...
 */
const pii = async (_ctx, data, config) => {
...
registry_1.defaultSpecRegistry.register('Contains PII', exports.pii, 'Checks that the text does not contain personally identifiable information (PII) such as SSNs, phone numbers, credit card numbers, etc., based on configured entity types.', 'text/plain', exports.PIIConfigRequired, undefined, { engine: 'Regex' });
