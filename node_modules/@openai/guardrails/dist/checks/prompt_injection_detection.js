...
exports.promptInjectionDetectionCheck = exports.PromptInjectionDetectionOutput = exports.PromptInjectionDetectionConfigRequired = exports.PromptInjectionDetectionConfig = void 0;
 ...
 */
exports.PromptInjectionDetectionConfig = zod_1.z.object({
    ...
    confidence_threshold: zod_1.z.number().min(0.0).max(1.0).default(0.7),
});
// Schema for registry registration (ensures all fields are provided)
exports.PromptInjectionDetectionConfigRequired = zod_1.z.object({
    model: zod_1.z.string(),
    confidence_threshold: zod_1.z.number().min(0.0).max(1.0),
});
/**
 ...
 */
exports.PromptInjectionDetectionOutput = llm_base_1.LLMOutput.extend({
    /** What the LLM action is doing */
    observation: zod_1.z.string().describe('What the LLM action is doing'),
    /** Specific evidence from conversation demonstrating the injection (required if flagged=true, otherwise null) */
    evidence: zod_1.z
        .string()
        .nullable()
        .describe('Specific evidence from the conversation that supports the decision. Required when flagged=true, otherwise null.'),
});
/**
...
`;
const STRICT_JSON_INSTRUCTION = 'Respond with ONLY a single JSON object containing the fields above. Do not add prose, markdown, or explanations outside the JSON. Example: {"observation": "...", "flagged": false, "confidence": 0.0, "evidence": null}';
/**
 * Prompt injection detection check for function calls, outputs, and responses.
 */
const promptInjectionDetectionCheck = async (ctx, data, config) => {
    try {
        const conversationHistory = safeGetConversationHistory(ctx);
        const parsedDataMessages = (0, conversation_1.normalizeConversation)((0, conversation_1.parseConversationInput)(data));
        if (conversationHistory.length === 0 && parsedDataMessages.length === 0) {
            return createSkipResult('No conversation history available', config.confidence_threshold, JSON.stringify([]));
        }
        const { recentMessages, actionableMessages, userIntent } = prepareConversationSlice(conversationHistory, parsedDataMessages);
        ...
        if (!userIntent.most_recent_message) {
            return createSkipResult('No LLM actions or user intent to evaluate', config.confidence_threshold, checkedText, userGoalText, actionableMessages, recentMessages);
        }
        if (actionableMessages.length === 0) {
            return createSkipResult('No actionable tool messages to evaluate', config.confidence_threshold, checkedText, userGoalText, actionableMessages, recentMessages);
        }
        ...
        return createSkipResult(`Error during prompt injection detection check: ${error instanceof Error ? error.message : String(error)}`, config.confidence_threshold, data);
    }
};
exports.promptInjectionDetectionCheck = promptInjectionDetectionCheck;
function safeGetConversationHistory(ctx) {
    try {
        const history = ctx.getConversationHistory?.();
        return (0, conversation_1.normalizeConversation)(history ?? []);
    }
    catch {
...
function prepareConversationSlice(conversationHistory, parsedDataMessages) {
...
function sliceMessagesAfterLatestUser(messages) {
...
function extractUserIntentFromMessages(messages) {
...
function extractActionableMessages(messages) {
...
function createSkipResult(observation, threshold, recentMessagesJson, userGoal = 'N/A', action = [], recentMessages = []) {
    return {
...
function formatUserGoal(userIntent) {
    if (!userIntent.most_recent_message) {
        return 'N/A';
    }
...
function buildAnalysisPrompt(userGoalText, recentMessages, actionableMessages) {
    const recentMessagesText = recentMessages.length > 0 ? JSON.stringify(recentMessages, null, 2) : '[]';
    const actionableMessagesText = actionableMessages.length > 0 ? JSON.stringify(actionableMessages, null, 2) : '[]';
...
async function callPromptInjectionDetectionLLM(ctx, prompt, config) {
    try {
        const result = await (0, llm_base_1.runLLM)(prompt, '', ctx.guardrailLlm, config.model, exports.PromptInjectionDetectionOutput);
        return exports.PromptInjectionDetectionOutput.parse(result);
    }
    catch {
...
registry_1.defaultSpecRegistry.register('Prompt Injection Detection', exports.promptInjectionDetectionCheck, "Guardrail that detects when tool calls or tool outputs contain malicious instructions not aligned with the user's intent. Parses conversation history and uses LLM-based analysis for prompt injection detection checking.", 'text/plain', exports.PromptInjectionDetectionConfigRequired, undefined, { engine: 'LLM', usesConversationHistory: true });
//# sourceMappingURL=prompt_injection_detection.js.map