...
exports.urls = exports.UrlsContext = exports.UrlsConfig = void 0;
...
function normalizeAllowedSchemes(value) {
...
function extractHostCandidate(url) {
    if (!url.includes('://')) {
        return null;
    }
...
function safeGetPort(parsed, scheme) {
    if (parsed.port) {
 ...
 */
function shouldBlockDueToPortMismatch(urlPort, urlParsed, allowedPort, allowedParsed, urlScheme, allowedScheme) {
    ...
    const allowedHasNonDefaultPort = allowedParsed.port &&
        (allowedPort !== DEFAULT_PORTS[allowedScheme]);
    ...
    const urlHasNonDefaultPort = urlParsed.port &&
        (urlPort !== DEFAULT_PORTS[urlScheme]);
 ...
 */
function isUrlAllowed(parsedUrl, allowList, allowSubdomains, hadScheme) {
    ...
    for (const allowedEntry of allowList) {
        ...
        if (cidrMatch) {
            ...
            if (prefix === 0 && network !== '0.0.0.0') {
                console.warn(`Warning: CIDR /0 prefix must use 0.0.0.0, not "${network}". Entry: "${normalizedEntry}"`);
                continue;
            }
            try {
                ...
                console.warn(`Warning: Invalid CIDR network address in allow list: "${normalizedEntry}" - ${error instanceof Error ? error.message : error}`);
            }
        ...
        try {
            ...
            console.warn(`Warning: Invalid URL in allow list: "${normalizedEntry}" - ${error instanceof Error ? error.message : error}`);
            continue;
        }
        ...
        const hostMatches = urlDomain === allowedDomain || (allowSubdomains && urlDomain.endsWith(`.${allowedDomain}`));
        if (!hostMatches) {
            continue;
        }
 ...
 */
const urls = async (ctx, data, config) => {
    const actualConfig = exports.UrlsConfig.parse(config || {});
    ...
    for (const urlString of detectedUrls) {
        ...
        else if (isUrlAllowed(parsedUrl, actualConfig.url_allow_list, actualConfig.allow_subdomains, hadScheme)) {
            allowed.push(urlString);
        }
        else {
            blocked.push(urlString);
...
registry_1.defaultSpecRegistry.register('URL Filter', exports.urls, 'URL filtering using regex + standard URL parsing with direct configuration.', 'text/plain', exports.UrlsContext, exports.UrlsConfig);
