...
exports.GuardrailMetricsCalculator = void 0;
/**
 * Calculates evaluation metrics from results.
 */
class GuardrailMetricsCalculator {
    /**
    ...
    calculate(results) {
        ...
        for (const name of guardrailNames) {
            // Calculate metrics
            const truePositives = results.filter((r) => r.expectedTriggers[name] && r.triggered[name]).length;
            const falsePositives = results.filter((r) => !r.expectedTriggers[name] && r.triggered[name]).length;
            const falseNegatives = results.filter((r) => r.expectedTriggers[name] && !r.triggered[name]).length;
            const trueNegatives = results.filter((r) => !r.expectedTriggers[name] && !r.triggered[name]).length;
            ...
            const precision = truePositives + falsePositives > 0 ? truePositives / (truePositives + falsePositives) : 0.0;
            const recall = truePositives + falseNegatives > 0 ? truePositives / (truePositives + falseNegatives) : 0.0;
...
exports.GuardrailMetricsCalculator = GuardrailMetricsCalculator;
