...
exports.BenchmarkMetricsCalculator = void 0;
/**
...
class BenchmarkMetricsCalculator {
     ...
     */
    calculateAdvancedMetrics(results, guardrailName, guardrailConfig) {
    ...
    extractLabelsAndScores(results, guardrailName) {
        const yTrue = [];
        const yScores = [];
        for (const result of results) {
            if (!(guardrailName in result.expectedTriggers)) {
                console.warn(`Guardrail '${guardrailName}' not found in expectedTriggers for sample ${result.id}`);
                continue;
            }
    ...
    calculateMetrics(yTrue, yScores) {
        const metrics = {};
        // Calculate ROC AUC
        try {
    ...
    precisionRecallCurve(yTrue, yScores) {
        // Sort by score descending
        const combined = yTrue.map((label, i) => ({ label, score: yScores[i] }));
        combined.sort((a, b) => b.score - a.score);
        const totalPositives = yTrue.filter((y) => y === 1).length;
    ...
    precisionAtRecall(precision, recall, targetRecall) {
        let bestPrecision = 0;
     ...
     */
    calculateAllGuardrailMetrics(results) {
        if (results.length === 0) {
            return {};
        }
...
exports.BenchmarkMetricsCalculator = BenchmarkMetricsCalculator;
//# sourceMappingURL=benchmark-calculator.js.map