...
**CRITICAL**: Subtotal calculation varies by `unit_base` field:

| `unit_base` | Formula | Example |
|-------------|---------|---------|
| `"unidad"` | `cantidad × sale_sin_iva` | 5 units × $20.77 = $103.85 |
| `"ml"` (metro lineal) | `cantidad × Length_m × sale_sin_iva` | 10 pcs × 3.0m × $20.77 = $623.10 |
| `"m²"` | `área_total × sale_sin_iva` | 300 m² × $33.21 = $9,963.00 |

**Important**:
- **Always check `unit_base`** before calculating totals
- **Do not multiply by `Length_m`** if `unit_base = "unidad"`
- **Use standardized field names**: `Thickness_mm`, `Length_m`
...
### Field: `specifications.thickness_mm` or `thickness_mm`
- Panel thickness in millimeters (30, 50, 80, 100, 150, etc.)
- `null` for accessories that work with any thickness
- **Use standardized name**: `Thickness_mm` (capital T, lowercase mm)

### Field: `length_m` or `Length_m`
- Product length in meters (typically 3.0 for standard profiles)
- Used in calculations when `unit_base = "ml"` (metro lineal)
- **Use standardized name**: `Length_m` (capital L, lowercase m)

### Field: `unit_base`
- **Critical for pricing calculations**
- Values: `"unidad"`, `"ml"`, `"m²"`, `"metro_lineal"`
- Determines how to calculate subtotals (see calculation logic above)
