...
try:
    ...
    from gpt_simulation_agent.agent_system.utils.mercadolibre_api import MercadoLibreAPIClient
except ImportError:
...
class AgenteIngestionAnalisis:
    ...
    def _init_database(self):
        ...
        # Índices
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_source ON ingestion_table(source)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_platform ON ingestion_table(platform)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_timestamp ON ingestion_table(timestamp)")
        ...
        if self.ml_client:
            try:
                ...
                for question in questions:
                    normalized = self.ml_client.normalize_interaction(question, "question")
                    record = IngestionRecord(
                ...
                for message in messages:
                    normalized = self.ml_client.normalize_interaction(message, "message")
                    record = IngestionRecord(
                        id=f"ml_{normalized['id']}",
                        source='mercadolibre',
                        platform='mercadolibre',
                    ...
                    if isinstance(data, list):
                        for item in data:
                            record = IngestionRecord(
                                ...
                                user_query=item.get('question', item.get('consulta', item.get('message', ''))),
                        ...
                        record = IngestionRecord(
                            ...
                            user_query=data.get('question', data.get('consulta', data.get('message', ''))),
                        ...
                        if isinstance(interactions, list):
                            for interaction in interactions:
                                record = IngestionRecord(
                                    ...
                                    platform='instagram',
                                    timestamp=interaction.get('timestamp', datetime.now().isoformat()),
                                    user_query=interaction.get('content', interaction.get('text', '')),
                                    chatbot_response=None,
                                    metadata={
                                        'type': interaction.get('type'),
                                        'user': interaction.get('user', {}),
                        ...
                        if isinstance(interactions, list):
                            for interaction in interactions:
                                record = IngestionRecord(
                                    ...
                                    platform='facebook',
                                    timestamp=interaction.get('timestamp', datetime.now().isoformat()),
                                    user_query=interaction.get('content', interaction.get('message', '')),
                                    chatbot_response=None,
                                    metadata={
                                        'type': interaction.get('type'),
                                        'user': interaction.get('user', {}),
        ...
        for quote_row in quotes:
            ...
            if self.motor:
                try:
                    ...
                    producto_match = re.search(r'(ISODEC|ISOPANEL|ISOROOF|ISOWALL)', user_query.upper())
    ...
    def _analizar_quote_input(self, query: str, metadata: Dict) -> Dict:
        ...
        required_fields = ['product_detected', 'espesor_detected', 'dimensiones_detected']
        optional_fields = ['luz_detected', 'fijacion_detected']
        
        required_score = sum(1 for field in required_fields if analysis[field]) / len(required_fields)
        optional_score = sum(1 for field in optional_fields if analysis[field]) / len(optional_fields)
        ...
        if analysis['completeness_score'] < 0.7:
            analysis['recommendations'].append('Input incompleto - requiere más información')
    ...
    def _analizar_social_query(self, query: str, source: str, metadata: Dict) -> Dict:
        ...
        question_words = ['?', 'cuanto', 'cuál', 'qué', 'cómo', 'dónde', 'cuándo', 'precio', 'costo']
        analysis['is_question'] = any(word in query_lower for word in question_words) or '?' in query
        
        # Detectar si requiere respuesta
        analysis['requires_response'] = analysis['is_question'] or any(
            word in query_lower for word in ['necesito', 'quiero', 'busco', 'consulta', 'información']
        )
        ...
        analysis['product_mentioned'] = any(producto in query_lower for producto in productos)
    ...
    def _analizar_respuesta(self, user_query: str, chatbot_response: str) -> Dict:
        ...
        accuracy_indicators = [
            'según', 'en la base', 'precio', 'costo', 'espesor',
            'autoportancia', 'fórmula', 'cálculo'
        ]
        
        accuracy_count = sum(1 for indicator in accuracy_indicators if indicator in response_lower)
        ...
        analysis['accuracy_score'] = max(0.0, analysis['accuracy_score'] - (vague_count * 0.2))
        
        # Completeness Score: ¿La respuesta es completa?
        # Verificar si responde a la pregunta
        is_question = '?' in user_query or any(
            word in query_lower for word in ['cuanto', 'cuál', 'qué', 'cómo', 'dónde', 'cuándo']
        )
        ...
        if is_question:
            ...
            if len(chatbot_response) > 20:  # Respuesta tiene contenido
                ...
                if any(word in response_lower for word in ['es', 'son', 'tiene', 'cuesta', 'vale']):
        ...
        query_positive = any(word in query_lower for word in ['gracias', 'excelente', 'bueno'])
        query_negative = any(word in query_lower for word in ['mal', 'problema', 'error'])
        
        response_positive = any(word in response_lower for word in ['gracias', 'excelente', 'bueno', 'perfecto'])
        response_negative = any(word in response_lower for word in ['disculpa', 'lamento', 'problema'])
    ...
    def _generar_resumen_respuestas(self, analyses: List[Dict]) -> Dict:
        ...
        sentiment_matches = sum(1 for a in analyses if a['analysis'].get('sentiment_match', False))
    ...
    def _generar_recomendaciones_generales(self, ingestion, quotes, social, responses) -> List[str]:
        """Genera recomendaciones generales basadas en todos los análisis"""
        recommendations = []
        
        # Recomendaciones de ingestion
        if ingestion['total_records'] == 0:
            recommendations.append('⚠️  No se encontraron registros para ingestion. Verificar fuentes de datos.')
        
        # Recomendaciones de cotizaciones
        if quotes.get('summary', {}).get('avg_completeness', 1.0) < 0.7:
            recommendations.append('📊 Mejorar completitud de inputs de cotizaciones - muchos inputs están incompletos')
        
        # Recomendaciones de redes sociales
        social_summary = social.get('summary', {})
        if social_summary.get('response_rate', 0.0) > 0.5:
            recommendations.append('📱 Alta tasa de consultas que requieren respuesta en redes sociales - priorizar atención')
        ...
        if response_summary.get('total_issues', 0) > 0:
            recommendations.append(f'💬 Revisar {response_summary["total_issues"]} issues detectados en respuestas')
