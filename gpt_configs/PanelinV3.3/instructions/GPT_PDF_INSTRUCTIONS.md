# GPT Instructions: PDF Quotation Generation (v2.1 Enhanced)

**Version**: 2.1 Enhanced (PanelinV3.3)  
**Updated**: 2026-02-10  
**Changes from v2.0**: Pre-generation validation, standardized nomenclature enforcement, graceful error handling with text fallback
...
### Error Handling (Enhanced v2.1)

PDF generation MUST implement graceful fallback:
...
try:
    ...
    print("⚠️ Logo BMC no encontrado. Generando PDF sin logo...")
    quotation_data['skip_logo'] = True
    pdf_path = generate_quotation_pdf(quotation_data, output_path)
    print(f"✅ PDF generado (sin logo): {pdf_path}")
except Exception as e:
    print(f"❌ Error generando PDF: {e}")
    print("Mostrando cotización en formato texto como alternativa...")
    # [Display text-based quotation with same data]
    print("Puede reintentar la generación con el comando /pdf")
```

### Pre-Generation Validation (New in v2.1)

Before calling `generate_quotation_pdf()`, validate ALL of the following:
...
def validate_quotation_data(data):
    ...
    if not data.get('products') or len(data['products']) == 0:
        errors.append("No hay productos en la cotización")
    
    # Validate each product
    for i, product in enumerate(data.get('products', [])):
        if not product.get('name'):
            errors.append(f"Producto {i+1}: falta nombre")
        if not product.get('unit_price_usd'):
            errors.append(f"Producto {i+1}: falta precio unitario")
        if not product.get('quantity'):
            errors.append(f"Producto {i+1}: falta cantidad")
        
        # Validate unit_base consistency
        unit_base = product.get('unit_base', 'unidad')
        if unit_base not in ('unidad', 'ml', 'm²'):
            errors.append(f"Producto {i+1}: unit_base inválido '{unit_base}'")
    
    return errors
```

If validation finds errors, present them to the user and request corrections before generating the PDF.

### Quality Checklist (Enhanced v2.1)
...
- [ ] Subtotals match `unit_base` formula (new v2.1)
- [ ] `validate_quotation_data()` returns no errors (new v2.1)
- [ ] Logo file path is accessible (new v2.1)
...
**Changes from v2.0**: Pre-generation validation, graceful error handling, logo fallback
