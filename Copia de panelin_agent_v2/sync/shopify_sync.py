...
class SyncConfig:
    ...
    kb_path: Path = Path(__file__).parent.parent / "config" / "panelin_truth_bmcuruguay.json"
    ...
    def __post_init__(self):
        self.webhook_secret = os.environ.get("SHOPIFY_WEBHOOK_SECRET", self.webhook_secret)
        self.alert_webhook_url = os.environ.get("SYNC_ALERT_WEBHOOK", self.alert_webhook_url)
...
def save_knowledge_base(kb: Dict[str, Any], config: SyncConfig, change_reason: str) -> bool:
    """
...
def map_shopify_to_kb_product(shopify_product: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """
    ...
    try:
        ...
        shopify_id = shopify_product.get("id") or shopify_product.get("admin_graphql_api_id")
        ...
        return {
            ...
            "stock_status": "available" if primary_variant.get("inventory_quantity", 0) > 0 else "out_of_stock",
    ...
    try:
        ...
        message = {
            ...
            "attachments": [{
                ...
                "fields": [
                    {"title": "Products Checked", "value": str(report["products_checked"]), "short": True},
                    {"title": "Discrepancies", "value": str(report["discrepancies_found"]), "short": True},
                    {"title": "Auto-Fixed", "value": str(report["auto_fixed"]), "short": True},
                    {"title": "Needs Review", "value": str(report["requires_review"]), "short": True}
                ]
            }]
        }
...
class ShopifySyncService:
    ...
    def handle_webhook(self, topic: str, payload: Dict[str, Any], hmac_header: str) -> SyncResult:
        """
