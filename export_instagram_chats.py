...
try:
    ...
    logger.warning(
        "Instagram API client not available. Only file processing will work."
    )
...
class InstagramChatExporter:
    ...
    def export_from_api(
        ...
        if not HAS_INSTAGRAM_API:
            raise ValueError(
                "Instagram API client not available. Install required dependencies."
            )
        ...
        logger.warning(
            "Instagram DMs via Graph API require Instagram Messaging API setup"
        )
        logger.info(
            "For personal accounts, use Instagram's 'Download Your Information' feature"
        )
        ...
        if dms:
            normalized_dms = [client.normalize_interaction(dm, "dm") for dm in dms]
            return self._save_chats(normalized_dms, "api_export")
        else:
            logger.info(
                "No DMs retrieved. Check API permissions or use file-based export."
            )
            return {"count": 0, "files": []}

    def export_from_files(
        self, instagram_data_path: str, include_archived: bool = True
    ) -> Dict:
        """
        ...
        if not data_path.exists():
            raise ValueError(
                f"Instagram data path does not exist: {instagram_data_path}"
            )
                ...
                if isinstance(chat_data, list):
                    ...
                    if "participants" in chat_data:
                        ...
                        parsed = self._parse_thread(
                            messages, participants, thread_title, chat_file.name
                        )
                        all_chats.extend(parsed)
                    elif "messages" in chat_data:
                        # Simple messages format
                        all_chats.extend(
                            self._parse_messages(chat_data["messages"], chat_file.name)
                        )
    ...
    def _parse_thread(
        ...
        for msg in messages:
            ...
            if "photos" in msg:
                parsed_msg["media"] = [
                    {"type": "photo", "uri": p.get("uri", "")} for p in msg["photos"]
                ]
            if "videos" in msg:
                parsed_msg["media"] = [
                    {"type": "video", "uri": v.get("uri", "")} for v in msg["videos"]
                ]
    ...
    def _parse_messages(self, messages: List[Dict], source_file: str) -> List[Dict]:
        ...
        for msg in messages:
            parsed_msg = {
                ...
                "timestamp": self._parse_instagram_timestamp(
                    msg.get("timestamp_ms", msg.get("timestamp"))
                ),
                "source_file": source_file,
                "metadata": {
    ...
    def _save_chats(self, chats: List[Dict], export_type: str) -> Dict:
        ...
        jsonl_file = (
            self.output_dir / f"instagram_chats_{export_type}_{timestamp}.jsonl"
        )
        ...
        for chat in chats:
            thread_key = chat.get(
                "thread_title",
                (
                    chat.get("participants", ["Unknown"])[0]
        ...
        threads_file = (
            self.output_dir
            / f"instagram_chats_by_thread_{export_type}_{timestamp}.json"
        )
        ...
        summary_file = (
            self.output_dir / f"export_summary_{export_type}_{timestamp}.json"
        )
...
def main():
    ...
    parser.add_argument(
        "--access-token", type=str, help="Instagram access token (for API export)"
    )
    ...
    if args.method in ["files", "both"]:
        ...
        try:
            ...
            logger.success(
                f"✅ Exported {results['files']['count']} messages from files"
            )
        except Exception as e:
    ...
    if args.method in ["api", "both"]:
        ...
        business_account_id = args.business_account_id or os.getenv(
        ...
        if not access_token:
            ...
            try:
                ...
                logger.success(
                    f"✅ Exported {results['api']['count']} messages from API"
                )
            except Exception as e:
