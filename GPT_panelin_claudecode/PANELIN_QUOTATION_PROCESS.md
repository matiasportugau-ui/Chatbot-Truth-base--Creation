...
2. Obtener:
   ...
   - Resistencia térmica
3. Consultar `accessories_catalog.json` para precios unitarios de accesorios (goteros, babetas, cumbreras, etc.)
4. Consultar `bom_rules.json` para reglas paramétricas de BOM por sistema constructivo
5. Verificar en Nivel 3 (`panelin_truth_bmcuruguay_web_only_v2.json`) si hay actualización de precio (pero usar Nivel 1 como base)
...
**Fuente**: Usar las fórmulas de `"formulas_cotizacion"` en `BMC_Base_Conocimiento_GPT-2.json` y las reglas paramétricas de `bom_rules.json`:
...
- Arandelas carrocero = PUNTOS * 1 (v6)
- Tortugas PVC = PUNTOS * 1 (v6)
- Fijaciones perfilería = ROUNDUP(ML_TOTALES_PERFILES / 0.30) (v6)
...
2. **IVA**: 22% YA incluido en los precios unitarios. **NO sumar IVA adicional.**
